        ...
        try:
                ...
                for i, po_record in enumerate(db_records['purchase_order_records']):
                    ...
                    try:
                        cursor.execute("""
                            ...
                            (po_id, po_number, item_number, vendor, order_quantity, unit_price, 
                             uom, total_value, created_at, updated_at, agent_reasoning_summary, sop_summary)
                            ...
                            ON CONFLICT (po_id) DO UPDATE SET
                                ...
                                agent_reasoning_summary = EXCLUDED.agent_reasoning_summary,
                                sop_summary = EXCLUDED.sop_summary
                        """, (
                            po_record.get('po_id'),
                            po_record.get('po_number'),
                            po_record.get('item_number'),
                            ...
                            po_record.get('agent_reasoning_summary'),
                            po_record.get('sop_summary')
                        ))
                ...
                for i, invoice_record in enumerate(db_records['invoice_records']):
                    ...
                    try:
                        cursor.execute("""
                            ...
                            (invoice_id, invoice_number, po_number, item_number, invoiced_quantity, 
                             unit_price, invoice_date, currency, created_at, updated_at, 
                             agent_reasoning_summary, sop_summary)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                        ...
                        """, (
                            ...
                            invoice_record.get('agent_reasoning_summary'),
                            invoice_record.get('sop_summary')
                        ))
            ...
            if 'goods_receipt_records' in db_records:
                ...
                for i, gr_record in enumerate(db_records['goods_receipt_records']):
                    ...
                    try:
                        cursor.execute("""
                            INSERT INTO goods_receipt_table 
                            (gr_id, gr_number, po_number, item_number, received_quantity, 
                        ...
                        """, (
                            ...
                            gr_record.get('agent_reasoning_summary'),
                            gr_record.get('sop_summary')
                        ))
                    ...
                    try:
                        cursor.execute("""
                            ...
                            (exception_id, exception_type, severity, status, po_number, 
                             item_number, created_at, updated_at, sop_connected, 
                             agent_reasoning_summary, sop_summary)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                        ...
                        """, (
                            ...
                            exception_record.get('item_number'),
                            ...
                            exception_record.get('agent_reasoning_summary'),
...
def _generate_database_records(po_number: str, po_record: dict, analysis_results: list, exceptions_found: list, match_status: str) -> dict:
    ...
    for i, item in enumerate(analysis_results):
        ...
        po_record_data = {
            "po_id": f"PO-{po_number}-{item_number}",
            "po_number": po_number,
        ...
        if item.get('has_invoice', False):
            invoice_record_data = {
                "invoice_id": f"INV-{po_number}-{item_number}",
        ...
        if item.get('has_gr', False):
            gr_record_data = {
                "gr_id": f"GR-{po_number}-{item_number}",
    ...
    for i, exception in enumerate(exceptions_found):
        exception_record_data = {
            "exception_id": f"EXC-{po_number}-{exception.get('item_number', '000')}-{i+1:03d}",
            "exception_type": exception.get('exception_type', 'Unknown'),
            "severity": exception.get('severity', 'Medium'),
            "status": "Pending",
            "po_number": po_number,
            "item_number": exception.get('item_number', '000'),
            "created_at": current_timestamp,
            "updated_at": current_timestamp,
            "sop_connected": f"SOP_{exception.get('exception_type', 'UNKNOWN').upper().replace(' ', '_')}_V1",
            "agent_reasoning_summary": exception.get('variance_data', {}).get('agent_reasoning_summary', f"Exception detected: {exception.get('exception_type', 'Unknown')} for PO {po_number}"),
