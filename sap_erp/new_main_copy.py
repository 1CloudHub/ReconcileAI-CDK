...
def create_sap_agent() -> Agent:
    ...
    tools = [fetch_s3_data, three_way_match_with_llm, sop_analysis, database_records]
...
"**Step 1 — Retrieve purchase, goods receipt and invoice documents for PO <PO_NUMBER>.**"

Call three_way_match_with_llm(needed_dir="needed_doc", output_file="three_way_match_result.json") and announce:
"**Step 2 — Perform three-way matching and identify discrepancies for PO <PO_NUMBER>.**"

Call sop_analysis(needed_dir="needed_doc", result_file="three_way_match_result.json") and announce:
"**Step 3 — Apply SOPs to determine severity, status and recommended actions for identified exceptions.**"

After Step 3, present the exception details in Markdown, one line per exception:
...
"**Step 4 — Prepare database records and persist exception details for downstream processing.**"

After Step 4, return a concise **final_summary** paragraph in Markdown describing overall outcome (how many exceptions, highest severity, and recommended next action).
